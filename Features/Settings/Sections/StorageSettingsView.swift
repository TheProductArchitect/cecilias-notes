...
struct StorageSettingsView: View {
    ...
    var body: some View {
        ...
        .task { await viewModel.loadStorageMetrics() }
        .alert("Clear Exported PDFs?", isPresented: $showClearExportsAlert) {
            Button("Cancel", role: .cancel) {}
            Button("Clear", role: .destructive) { Task { await clearExports() } }
        } message: {
        ...
        .alert("Clear Audio Recordings?", isPresented: $showClearAudioAlert) {
            Button("Cancel", role: .cancel) {}
            Button("Delete Recordings", role: .destructive) { Task { await clearAudio() } }
        } message: {

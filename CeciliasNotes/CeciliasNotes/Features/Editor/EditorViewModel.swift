...
final class EditorViewModel: ObservableObject {
    ...
    // MARK: - Stroke-ended hook

    /// Called from the canvas coordinator on `canvasViewDidEndUsingTool`.
    /// Highlighter-family strokes that cross selectable PDF text are
    /// converted to `PageElement(.highlight)` rows; every other stroke
    /// is left exactly as drawn. (Shape Assist — automatic stroke→shape
    /// conversion — was removed 2026-07: it repeatedly rewrote
    /// handwriting into geometry, "my writing gets undone by itself".)
